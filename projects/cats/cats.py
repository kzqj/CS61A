...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    correct_words = [
        word for word in word_list if diff_function(typed_word, word, limit) <= limit
    ]
    if len(correct_words) == 0:
        return typed_word
    return min(correct_words, key=lambda word: diff_function(typed_word, word, limit))
    # END PROBLEM 5
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(timestamps_per_player)):
        ...
        for j in range(1, len(timestamps_per_player[i])):
            tmp.append(timestamps_per_player[i][j] - timestamps_per_player[i][j - 1])
        times.append(tmp)
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if times[j][i] < times[min_idx][i]:
                min_idx = j
        result[min_idx].append(words[i])
